    ...
    <div className="md:col-span-2 bg-white rounded-2xl border border-slate-200 shadow-[0_2px_8px_-2px_rgba(0,0,0,0.05)] flex flex-col overflow-hidden">
      ...
      <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-gradient-to-r from-slate-50 to-white">
        <div className="flex items-center gap-3">
          <div className={`w-2 h-8 rounded-full ${result.overallOk ? "bg-emerald-400" : "bg-rose-400"}`} />
          <div>
            <h3 className="font-bold text-slate-700 text-sm">Tiêu chí {planLabel}</h3>
            {totalCount > 0 && (
              <p className="text-[11px] text-slate-400 font-medium mt-0.5">
                {passCount}/{totalCount} tiêu chí đạt
              </p>
            )}
          </div>
        </div>
              ...
              return (
                <div
                  ...
                  className={`md:col-span-2 rounded-xl p-4 border transition-all ${
                    c.ok ? "bg-emerald-50/60 border-emerald-200" : "bg-rose-50/40 border-rose-200"
                  }`}
                >
                  <div className="flex items-center justify-between mb-3">
                    <span className={`text-xs font-black uppercase tracking-wide ${c.ok ? "text-emerald-700" : "text-rose-700"}`}>
                      {c.label}
                    </span>
                    {c.ok ? (
                      ...
                      <span className="text-[10px] text-rose-500 font-bold bg-white px-2.5 py-1 rounded-lg border border-rose-200 shadow-sm">Chưa đạt</span>
                    )}
                  </div>
                      ...
                      return (
                        ...
                        >
                          <div className="flex justify-between">
                            ...
                            <span className="font-extrabold tabular-nums">{o.got}/{o.req}</span>
                          </div>
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  <span className={`truncate text-xs font-semibold leading-snug ${isPass ? "text-emerald-800" : "text-slate-600"}`}>
                    {c.label}
                  </span>
                  <div className="flex items-center gap-2 text-xs min-w-fit">
                    <span className={`font-extrabold tabular-nums px-2 py-0.5 rounded-md ${isPass ? "bg-white text-emerald-600 shadow-sm border border-emerald-100" : "bg-white text-rose-500 border border-rose-100"}`}>
                      {got}/{req}
                    </span>
                    <div className={`rounded-full p-0.5 ${isPass ? "text-emerald-500 bg-emerald-100" : "text-rose-400 bg-rose-100"}`}>
