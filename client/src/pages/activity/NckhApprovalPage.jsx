      ...
      <div className="p-5 flex-1 flex flex-col">
        ...
        <div className="space-y-1.5 text-sm text-gray-500 mb-4">
          <div className="flex items-center gap-2">
            <Person sx={{ fontSize: 15 }} className="text-gray-400 flex-shrink-0" />
          ...
          <div className="text-xs text-gray-400">Năm học: {item.academicYear}</div>
        </div>
...
export default function NckhApprovalPage() {
  ...
  const loadUserDirectory = async () => {
    try {
      ...
      try {
        ...
        source = res?.data?.result || res?.data?.content || res?.data || res?.result || res?.content || res || [];
      } catch { source = []; }
      if (!Array.isArray(source) || source.length === 0) {
        try {
          const res2 = await userService.getAllUsers({ page: 0, size: 500 });
          source = res2?.data?.result || res2?.data?.content || res2?.data || res2?.result || res2?.content || res2 || [];
        } catch { source = []; }
      }
      const map = {};
  ...
  useEffect(() => { loadPending(); }, [selectedYear, typeFilter]);
  ...
  const filtered = pendingItems.filter((item) => {
    ...
    return name.includes(normalizedKw) || (item.title || "").toLowerCase().includes(normalizedKw);
  });
      ...
      <section className="bg-gradient-to-r from-mainColor to-[#154c6e] text-white py-12">
        <div className="max-w-6xl mx-auto px-4">
          <div className="max-w-3xl">
            <h1 className="text-3xl md:text-4xl font-bold mb-3">Duyệt khai báo NCKH</h1>
            <p className="text-lg text-white/80">
              Xét duyệt các khai báo hoạt động nghiên cứu khoa học đang chờ xử lý
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-6xl mx-auto px-4 py-6">
        ...
        <div className="bg-white rounded-lg shadow-sm mb-6 px-5 py-4 flex flex-wrap gap-4 items-end">
          ...
          <label className="flex flex-col gap-1">
            <span className="text-xs font-bold text-gray-500 uppercase">Năm học</span>
            <input
              type="number"
              value={selectedYear}
          ...
          <label className="flex flex-col gap-1">
            <span className="text-xs font-bold text-gray-500 uppercase">Loại hoạt động</span>
            <select
              value={typeFilter}
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-white rounded-lg shadow-sm">
            <CheckCircle sx={{ fontSize: 48 }} className="text-green-400 mb-3" />
            <p className="text-gray-500 text-base font-medium">Không có khai báo nào chờ duyệt</p>
            <p className="text-sm text-gray-400 mt-1">Tất cả khai báo đã được xử lý</p>
          </div>
        ) : (
